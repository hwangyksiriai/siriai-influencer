...
const chip = (active: boolean): React.CSSProperties => ({
  ...
  border: active ? `1.5px solid ${T.accent}` : `1.5px solid ${T.line}`,
  cursor: 'pointer', transition: 'all .15s', fontFamily: 'inherit',
  background: active ? T.accent : T.surface, color: active ? T.accentInk : T.ink2,
})
...
function PwCheck({ ok, children }: { ok: boolean; children: React.ReactNode }) {
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 12, fontWeight: 600, color: ok ? T.sageInk : T.ink3, transition: 'color .2s' }}>
  ...
  if (done) {
    return (
      <div className="au-step" style={{
        minHeight: '100vh', background: T.bg, fontFamily: T.fontUI,
        display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
        padding: '40px 28px', textAlign: 'center', position: 'relative', overflow: 'hidden',
      }}>
        <div aria-hidden style={{ position: 'absolute', top: -100, left: '50%', transform: 'translateX(-50%)', width: 380, height: 380, borderRadius: '50%', background: `radial-gradient(circle, ${T.sage} 0%, transparent 68%)`, opacity: 0.45, pointerEvents: 'none' }} />
        <div style={{ width: 76, height: 76, borderRadius: '50%', background: T.sage, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 22, position: 'relative' }}>
          <svg width="34" height="34" viewBox="0 0 24 24" fill="none"><path d="M5 12.5l4.5 4.5L19 7" stroke={T.sageInk} strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" /></svg>
        </div>
        <h1 style={{ fontFamily: T.fontDisplay, fontSize: 23, fontWeight: 700, letterSpacing: '-0.02em', color: T.ink, marginBottom: 12 }}>가입 신청이 완료됐어요</h1>
        <p style={{ fontSize: 14.5, color: T.ink2, lineHeight: 1.7, marginBottom: 10 }}>
          인스타그램 계정 확인 및 검토 후 승인돼요.<br />보통 <strong style={{ color: T.ink }}>1~2일 이내</strong>에 처리됩니다.
        </p>
        <p style={{ fontSize: 13, color: T.ink3, marginBottom: 30 }}>승인되면 가입하신 이메일로 바로 로그인할 수 있어요.</p>
        <Link href="/login" style={{ textDecoration: 'none', width: '100%', maxWidth: 320 }}>
          <button style={{ width: '100%', background: T.accent, color: T.accentInk, border: 'none', borderRadius: 999, padding: '16px', fontSize: 15, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
            로그인 화면으로
          </button>
        </Link>
      </div>
  ...
  return (
    <div className="dvh-screen" style={{
      ...
      <div style={{ marginBottom: 26 }}>
        {step === 1 ? (
          <Link href="/login" aria-label="로그인으로 돌아가기" style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 14, color: T.ink3, textDecoration: 'none', padding: '6px 0', fontWeight: 600 }}>
          ...
          <button type="button" onClick={() => { setError(''); setStep(1) }} aria-label="이전 단계로"
            style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 14, color: T.ink3, background: 'none', border: 'none', cursor: 'pointer', padding: '6px 0', fontWeight: 600, fontFamily: 'inherit' }}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M15 5l-7 7 7 7" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>
            이전
          </button>
        )}
        <h1 style={{ fontFamily: T.fontDisplay, fontSize: 26, fontWeight: 700, letterSpacing: '-0.02em', color: T.ink, marginTop: 12, marginBottom: 4 }}>
          {step === 1 ? '계정 만들기' : '프로필 입력'}
        </h1>
        <p style={{ fontSize: 13.5, color: T.ink3, margin: 0 }}>
          {step === 1 ? '로그인에 사용할 이메일과 비밀번호를 정해 주세요.' : '활동 정보를 알려주시면 검토 후 승인해 드려요.'}
        </p>
      </div>

      {/* 스텝 인디케이터 */}
      <div style={{ display: 'flex', gap: 8, marginBottom: 28 }} aria-label={`${STEPS.length}단계 중 ${step}단계`}>
        {STEPS.map(s => (
          <div key={s.n} style={{ flex: 1 }}>
            <div style={{ height: 4, borderRadius: 2, background: step >= s.n ? T.accent : T.line, transition: 'background .3s' }} />
            <div style={{ fontSize: 11, fontWeight: 600, color: step >= s.n ? T.ink : T.ink3, marginTop: 6 }}>{s.n}. {s.label}</div>
          </div>
        ))}
      </div>
        ...
        {step === 1 && (
          <div className="au-step" style={{ display: 'flex', flexDirection: 'column', gap: 16, flex: 1 }}>
            <div>
              ...
              <input id="su-email" className="au-input" style={inp} type="email" inputMode="email" autoComplete="email"
              ...
              {form.email.length > 3 && !emailValid && (
                <p style={{ fontSize: 12, color: T.danger, margin: '6px 2px 0', fontWeight: 500 }}>이메일 형식을 확인해 주세요.</p>
              )}
            </div>
            <div>
              <label htmlFor="su-pw" style={lbl}>비밀번호</label>
              <div style={{ position: 'relative' }}>
                <input id="su-pw" className="au-input" style={{ ...inp, paddingRight: 64 }}
            ...
            {error && (
              <div className="au-error" role="alert" style={{ display: 'flex', alignItems: 'flex-start', gap: 8, background: 'rgba(176,71,59,0.07)', border: '1px solid rgba(176,71,59,0.18)', borderRadius: 12, padding: '11px 13px' }}>
            ...
            <button type="submit" disabled={!form.email || !form.password}
              style={{
                background: T.accent, color: T.accentInk, borderRadius: 999, padding: '16px', fontSize: 15.5, fontWeight: 700,
                border: 'none', cursor: !form.email || !form.password ? 'default' : 'pointer', marginTop: 'auto',
                opacity: !form.email || !form.password ? 0.45 : 1, transition: 'opacity .2s', fontFamily: 'inherit',
              }}>
              다음
            </button>
          </div>
          ...
          <div className="au-step" style={{ display: 'flex', flexDirection: 'column', gap: 16, flex: 1 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <div>
                ...
                <input id="su-name" className="au-input" style={inp} type="text" autoComplete="name"
              ...
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  <span aria-hidden style={{ position: 'absolute', left: 16, top: '50%', transform: 'translateY(-50%)', fontSize: 16, color: T.ink3, fontWeight: 600 }}>@</span>
                  <input id="su-handle" className="au-input" style={{ ...inp, paddingLeft: 36 }} type="text" autoComplete="off"
            ...
            <div>
              ...
              <select id="su-region" className="au-input" style={{ ...inp, color: form.region ? T.ink : T.ink3 }} value={form.region} onChange={e => set('region', e.target.value)}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <input className="au-input" style={inp} type="text" inputMode="url" autoCapitalize="none" autoCorrect="off" spellCheck={false} placeholder="유튜브 채널 링크" value={form.youtube_url} onChange={e => set('youtube_url', e.target.value)} />
                <input className="au-input" style={inp} type="text" inputMode="url" autoCapitalize="none" autoCorrect="off" spellCheck={false} placeholder="블로그 링크" value={form.blog_url} onChange={e => set('blog_url', e.target.value)} />
                <input className="au-input" style={inp} type="text" inputMode="url" autoCapitalize="none" autoCorrect="off" spellCheck={false} placeholder="틱톡 링크" value={form.tiktok_url} onChange={e => set('tiktok_url', e.target.value)} />
              </div>
            </div>
            ...
            <button type="submit" disabled={loading || !form.name.trim() || !form.handle.trim()}
              style={{
                ...
                opacity: loading || !form.name.trim() || !form.handle.trim() ? 0.45 : 1, transition: 'opacity .2s', fontFamily: 'inherit',
              }}>
