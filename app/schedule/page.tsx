  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data }) => {
      ...
      const { data: applications } = await supabase
        ...
        .in('status', ['pending', 'in_progress', 'scouted'])
        ...
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {apps.map(a => {
              ...
              const ddayColor = dday === '완료' ? 'rgba(33,26,51,.4)' : dday.startsWith('D-1') || dday === 'D-day' ? '#e03' : '#211A33'
              return (
                <div key={a.id} style={{ background: 'rgba(255,255,255,.7)', border: '1px solid rgba(33,26,51,.1)', borderRadius: 14, padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ width: 48, height: 48, background: '#F3EEE2', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, border: '1px solid rgba(33,26,51,.1)' }}>
                    <span style={{ fontSize: 10, fontWeight: 700, fontFamily: "'JetBrains Mono',monospace", letterSpacing: '.06em', color: 'rgba(33,26,51,.4)' }}>{a.campaigns?.content_type?.slice(0, 2)}</span>
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ fontSize: 11, color: 'rgba(33,26,51,.5)' }}>
                      {a.campaigns?.content_type} 업로드
                      {' '}<span style={{ fontWeight: 700, color: statusColor[a.status] }}>{statusLabel[a.status]}</span>
                    </p>
                  </div>
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    {dday && (
                      <span style={{ display: 'inline-block', background: dday === '완료' ? 'rgba(33,26,51,.1)' : '#211A33', color: dday === '완료' ? 'rgba(33,26,51,.4)' : '#F3EEE2', fontSize: 10, fontWeight: 700, padding: '3px 8px', borderRadius: 6 }}>
                        {dday}
                      </span>
                    )}
                    <p style={{ fontSize: 11, color: 'rgba(33,26,51,.4)', marginTop: 4 }}>
                      {a.campaigns?.upload_end}
                    </p>
                  </div>
                </div>
              )
